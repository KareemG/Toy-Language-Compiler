...
public class CodeGen extends ASTVisitor.Default
{
	...
	void Initialize() {
		...
		actions.put(42, (s, self) -> {
			this.intermediate_code.add(new IR(IR.BF, new IR.Operand(IR.Operand.REGISTER, this.result_stack.pop().shortValue()), new IR.Operand(IR.Operand.PATCH, (short) 0)));
		});
		...
		actions.put(44, (s, self) -> {
			this.intermediate_code.add(new IR(IR.COND_REPEAT, new IR.Operand(IR.Operand.NONE, result_stack.pop().shortValue())));
		});
		...
		actions.put(51, (s, self) -> {
			...
			this.intermediate_code.add(new IR(Machine.PRINTI, new IR.Operand(IR.Operand.REGISTER, result_stack.pop().shortValue())));
		...
		actions.put(57, (s, self) -> {
			...
			ExitStmt stmt = (ExitStmt) s.get(0);
			short level = stmt.getLevel() <= 1 ? 1 : stmt.getLevel().shortValue();
			this.intermediate_code.add(new IR(IR.COND_EXIT, new IR.Operand(IR.Operand.NONE, level), new IR.Operand(IR.Operand.NONE, result_stack.pop().shortValue())));
		});
	...
	void Finalize() throws MemoryAddressException {
		...
		try
		{
			...
			translator.initialize((short) this.reg_offset);
