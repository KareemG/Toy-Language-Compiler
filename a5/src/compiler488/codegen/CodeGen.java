...
public class CodeGen extends ASTVisitor.Default
{
	...
	void Initialize() {
		...
		actions.put(40, (s, self) -> {
			...
			this.intermediate_code.add(new IR(IR.BRANCH, new IR.Operand(IR.Operand.NONE, (short) 1), new IR.Operand(IR.Operand.PATCH_TRUE, (short) 0)));
		});
		...
		actions.put(42, (s, self) -> {
			assert(s.get(0) instanceof IfStmt);
			this.intermediate_code.add(new IR(IR.BRANCH, new IR.Operand(IR.Operand.REGISTER, this.result_stack.pop().shortValue()), null, new IR.Operand(IR.Operand.PATCH_FALSE, (short) 0)));
		});
		...
		actions.put(77, (s, self) -> {
			...
			this.intermediate_code.add(new IR(IR.ASSIGN, new IR.Operand(IR.Operand.REGISTER, (short) lhs), new IR.Operand(IR.Operand.REGISTER, (short) rhs)));
		});
		...
		actions.put(78, (s, self) -> {
			...
			this.intermediate_code.add(new IR(IR.ASSIGN, new IR.Operand(IR.Operand.REGISTER, (short) target_register), new IR.Operand(IR.Operand.NONE, Machine.MACHINE_FALSE)));
		...
		actions.put(79, (s, self) -> {
			assert(s.get(0) instanceof BoolConstExpn);
			int target_register = this.reg_offset ++;
			this.intermediate_code.add(new IR(IR.ASSIGN, new IR.Operand(IR.Operand.REGISTER, (short) target_register), new IR.Operand(IR.Operand.NONE, Machine.MACHINE_TRUE)));
			this.result_stack.add(target_register);
		});
		...
		actions.put(80, (s, self) -> {
			...
			int target_register = this.reg_offset ++;
			this.intermediate_code.add(new IR(IR.ASSIGN, new IR.Operand(IR.Operand.REGISTER, (short) target_register), new IR.Operand(IR.Operand.NONE, ((IntConstExpn) s.get(0)).getValue().shortValue())));
			this.result_stack.add(target_register);
		});
				...
				switch(ir.opcode)
				{
					...
					case IR.BRANCH:
					{
						if(!ir.op1.is_register())
						{
							...
							if(ir.op1.get_value() != 0) {
								addr = ir.op2.get_value();
								if(ir.op2.needs_true_patch()) {
									true_stack.add(addr);
								}
							} else {
								addr = ir.op3.get_value();
								if(ir.op3.needs_false_patch()) {
									false_stack.add(addr);
								}
							}

							writeMemory(this.startMSP++, Machine.PUSH);
							writeMemory(this.startMSP++, addr);
							writeMemory(this.startMSP++, Machine.BR);
						}
						else
						{
							writeMemory(this.startMSP++, Machine.ADDR);
							...
							{
								writeMemory(this.startMSP++, Machine.PUSH);
								writeMemory(this.startMSP++, Machine.MACHINE_FALSE);
								writeMemory(this.startMSP++, Machine.EQ);

								short addr = ir.op2.get_value();
								writeMemory(this.startMSP++, Machine.PUSH);
							...
							{
								System.out.println("TODO: IMPLEMENT BRANCH WITH TRUE AND FALSE");
								throw new Exception();
							}
							writeMemory(this.startMSP++, Machine.BF);
						}
						break;
					}
